...
public class TitrationScenario : MonoBehaviour/*, ITrackableEventHandler*/
{
    ...
    [SerializeField] Transform curveIndicator;
    ...
    [Space]
    [Header("MoleculeReferences")]
    [SerializeField] Transform ammoniaTopH;
    [SerializeField] Transform ammoniaTopCl;
    ...
    [Space]
    [Header("Misc Values")]
    [SerializeField] AnimationCurve graphBallCurve;
    [SerializeField] bool graphBallMoved;
    [Space]
    ...
    void Update() {
        //curveIndicator.localPosition = new Vector3(curveIndicator.localPosition.x - 0.01f, graphBallCurve.Evaluate(-(curveIndicator.localPosition.x - 0.01f)), 0);
        ...
        if (waitTime > 0) {
            ...
            switch (step) {
                ...
                case 7:
                    if (!restarted) {
                        waitTime += audioPlay.Tracks[4].length;
                        audioPlay.PlayAudio();
                    }
                    step++;
                    break;
    ...
    void Intro() {
        ...
        wholeBreakpointCurve.localScale = Vector3.zero;
    ...
    void MoveBallAlongCurve() {
        TurnTowardsCamera(wholeBreakpointCurve,0);

        inStepWaitTime -= Time.deltaTime;

        if (!Input.GetMouseButtonDown(0) && Input.GetMouseButton(0) && inStepWaitTime <= 0) {
            dialogueSource.Stop();
            graphBallMoved = true;
            curveIndicator.localPosition = new Vector3(Mathf.Clamp(curveIndicator.localPosition.x - GlobalFunctions.TouchSlideCalc(0.025f).x,-16.4f,0),curveIndicator.localPosition.y,0);
            curveIndicator.localPosition = new Vector3(curveIndicator.localPosition.x,graphBallCurve.Evaluate(-curveIndicator.localPosition.x),0);
        } else if (graphBallMoved) {
            graphBallMoved = false;
            if (Mathf.Abs(curveIndicator.localPosition.x + 4.5f) < 0.5f) {
                step = 2;
            } else if (Mathf.Abs(curveIndicator.localPosition.x + 5.8f) < 0.5f) {
                step = 3;
            } else if (Mathf.Abs(curveIndicator.localPosition.x + 10.54f) < 0.5f) {
                step = 4;
            } else if (Mathf.Abs(curveIndicator.localPosition.x + 16.4f) < 0.1f) {
                step = 5;
            }
        }
    }
    ...
    void CurvePoint1() {
        Debug.Log("CurvePoint1");
        curveIndicator.localPosition = new Vector3(-4.5f,graphBallCurve.Evaluate(4.5f),0);

        StartCoroutine(Monochloramine());
    ...
    void CurvePoint2() {
        Debug.Log("CurvePoint2");
        curveIndicator.localPosition = new Vector3(-5.8f,graphBallCurve.Evaluate(5.8f),0);
        inStepWaitTime = 1f;
        step = 1;
    }

    void CurvePoint3() {
        Debug.Log("CurvePoint3");
        curveIndicator.localPosition = new Vector3(-10.54f,graphBallCurve.Evaluate(10.54f),0);
        inStepWaitTime = 1f;
        step = 1;
    }

    void CurvePoint4() {
        Debug.Log("CurvePoint4");
        StartCoroutine(HideCurve());
        curveIndicator.localPosition = new Vector3(-16.4f,graphBallCurve.Evaluate(16.4f),0);
        step++;
    }
    ...
    void DisplayTitrator() {
        ...
        if (restarted)
            ...
            waitTime += audioPlay.Tracks[3].length;
            audioPlay.PlayAudio(3);
        }
        step++;
    }
    ...
    public void Restart() {
        ...
        runCount = 1;
        dropperRotSpeed = 70;
        bottomButtonPivot.transform.localEulerAngles = new Vector3(0,0,knobOffRot);

        curveIndicator.localPosition = Vector3.zero;
        ...
        pipetValueContainer.parent = pipetteWater.parent;
    ...
    public bool KnobMovement(GameObject ObjToMov,float knobValue,float speed) {
        ...
        if (Mathf.Abs(knobEulerAngle - knobValue) < 7.5f)  // This makes sure that the knob will stop //at its desired position
        {
            knobRotIndex++;
        }
        return false;
    ...
    IEnumerator Monochloramine() {
        ...
        while (true) {
            ...
            ammoniaTopCl.GetChild(0).GetChild(0).localScale = Vector3.Lerp(Vector3.one,Vector3.zero,currentTime * 2);
        ...
        Vector3 clStartPos = ammoniaTopCl.localPosition;

        startTime = Time.time;
        while (true) {
            currentTime = Time.time - startTime; //sets current time to current time in program

            ammoniaTopCl.localPosition = Vector3.Lerp(clStartPos,new Vector3(0,clStartPos.y,0),currentTime * 2);
            ammoniaTopH.localPosition = Vector3.Lerp(new Vector3(0,clStartPos.y,0),new Vector3(-clStartPos.x,clStartPos.y,0),(currentTime * 2) - 0.25f);
        ...
        endScale = ammoniaTopH.localScale;

        startTime = Time.time;
        while (true) {
            currentTime = Time.time - startTime; //sets current time to current time in program

            ammoniaTopH.localScale = Vector3.Lerp(endScale,Vector3.zero,currentTime * 2);
    ...
    IEnumerator DisplayCurve() {
        ...
        while (true) {
            ...
            wholeBreakpointCurve.localScale = Vector3.Lerp(Vector3.zero,Vector3.one * 0.04f,currentTime / curveDisplayTime);
    ...
    IEnumerator HideCurve() {
        ...
        while (true) {
            ...
            wholeBreakpointCurve.localScale = Vector3.Lerp(Vector3.one * 0.04f,Vector3.zero,currentTime / curveDisplayTime);
            //scales object up to 100, the end bit here divides the current time by the animation time. Once they both match, anim over
